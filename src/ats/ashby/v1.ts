...
export class AshbyAdapterV1 implements ApplicationAdapter {
  ...
  setApprovedFillPlan(plan: ApprovedFillPlan, profile?: PublicProfile): void {
    // profile is optional only for cross-ATS call-shape parity — Ashby
    // cannot fill without it (full-name composition), so fail closed.
    if (!profile) {
      throw new Error(
        "Ashby fill requires the public profile for full-name composition — pass profile to setApprovedFillPlan",
      );
    }
